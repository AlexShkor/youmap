...
namespace YouMap.Documents.Lucene
{
    ...
    public abstract class LuceneIndexService<T> where T : class
    {
        ...
        protected LuceneIndexService(Settings settings)
        {
            _indexesDirectory = settings.LuceneIndexesDirectory;
            _flushAfter = 1;
        ...
        public void Flush(bool flushAll = false)
        {
            lock (SyncRoot)
            {
                ...
                {
                    writer.UpdateDocument(new Term("_id", document.Get("_id")), document);
                }

                var terms = _idsToDelete.Select(x => new Term("_id", x)).Where(x => x != null).ToArray();
        ...
        private Directory GetIndexDirectory(string indexName)
        {
